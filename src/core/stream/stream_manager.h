...
private:
	...
	a_hashset<uuids::uuid> object_removal_swep_;// 上帧收集到的
	...
	void _flush_pending_db_ops(); // 每帧末数据库同步
	void _on_query_result(const a_hashmap<uuids::uuid, ObjectData> &db_objects); // 异步查询完成回调
};
