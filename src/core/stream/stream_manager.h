...
class StreamManager : public Node3D {
	...
	a_hashmap<String, ObjectData> object_registry;
	// 父子关系辅助：parent_uuid -> set<child_uuid>
	a_hashmap<String, a_hashset<String>> children_map;
	// 待处理的数据库写操作（在主线程收集，定期批量提交）
	a_hashset<String> to_upsert;
	a_hashset<String> to_remove;

	// 场景加载管理
	ObjectSceneCache scene_cache;
...
public:
	...
	void update_object(const StreamObjectNode *obj);
