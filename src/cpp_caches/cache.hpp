...
public:
	...
	explicit fixed_sized_cache(
			...
			cache_policy{ policy }, max_cache_size{ max_size == 0 ? 1 : max_size }, on_erase_callback{ on_erase } {
	}
	...
	/**
	 * \brief Remove the element by key and return its value (if found).
	 * \param[in] key Key to pop
	 * \return Pair of (value, success_flag). If not found, value is default-constructed and success_flag is false.
	 * \note Unlike Remove(), this method does NOT invoke the on_erase_callback.
	 */
	std::pair<value_type, bool> Pop(const Key &key) {
		operation_guard lock{ safe_op };
		...
		value_type value = std::move(elem->second);
	...
	/**
	 * \brief Clear the cache, calling the on_erase callback for each evicted element.
	 * \note Unlike Clear(), this method invokes the user-provided callback.
	 */
	void ClearWithCallback() {
		operation_guard lock{ safe_op };
		while (!cache_items_map.empty()) {
			Erase(cache_items_map.begin());
		}
	}
